...
const UserOrder = () => {
  ...
  <div className="max-w-4xl mx-auto bg-gray-800 p-6 rounded shadow">
    <h1 className="text-2xl font-semibold mb-4">  {orders.length === 0 ? 'No Order Found - Check The Deivered page !!' : 'Ordered Items'}</h1>
    <div className="space-y-4">
      {orders.map((data,orderIndex) => (
          uid === data.userid && (
        <div key={orderIndex} className="p-4 border rounded shadow dark:bg-gray-700 mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="font-semibold">Order ID: {data.orderId}</span>
            <label className="flex items-center">
              <span className="font-semibold text-red-800">Order Pending</span>
            </label>
          </div>
          ...
          <div className="w-full h-48 overflow-y-scroll">
                    ...
                    {data.cart.map((item, itemIndex) => (
                      <tr key={itemIndex}>
                        <td className="py-2 px-4">
                          <img src={item.imageUrl} alt="Item Image" className="w-16 h-16 object-cover rounded" />
                        </td>
            ...
            <div>
              ...
              <p>Name: {data.addressInfo.name}</p>
              <p>Address: {data.addressInfo.address}</p>
              <p>Phone: (+91) {data.addressInfo.phoneNumber}</p>
              <p>Pincode: {data.addressInfo.pincode}</p>
              <p>Date: {data.addressInfo.date}</p>
            </div>
          </div>
          <div className="mt-4">
            <h2 className="text-lg font-semibold">Payment Info</h2>
            <p>Payment ID: {data.paymentId}</p>
            <p>Total Payment: ₹{data.totalPrice}</p>
